...
if($profesor!=0){
$sql_prof = "SELECT nombre_completo FROM docente WHERE id_docente='".$profesor."';";
$result=$conexion->query($sql_prof);
while($row1=$result->fetch_assoc()){
    $nombre_profesor=$row1['nombre_completo'];
}}
else{$nombre_profesor=$_POST["profesor"];}
...
$sexo=$_POST["sexo"];
if($sexo==0){
    $sql_sexo="SELECT sexo.sexo FROM alumno, sexo WHERE sexo.id_sexo=alumno.N_sexo AND alumno.id_alumno=".$nro_inscrito.";";
    $res_sexo=$conexion->query($sql_sexo);
    while($row_sexo=$res_sexo->fetch_assoc()){
        $sexo=$row_sexo['sexo'];
    }
}
...
$pdf->Cell(105,$alto,utf8_decode('PROFESOR: '.strtoupper($nombre_profesor)),1,0,'L',0);
...
$pdf->SetFont('Arial','',$letra); $pdf->Cell(105,$alto,utf8_decode(strtoupper($sexo)),1,0,'L',0);
...
$pdf->SetFont('Arial','B',$letra);
$pdf->MultiCell(90,$alto,utf8_decode(''.$dia),1,'L',false);$pdf->Ln();
$x = $pdf->GetX();
$y = $pdf->GetY();

        $pdf->SetXY($x+106, $y-11);
        
    $pdf->MultiCell(35,$alto,utf8_decode(''.$desde),1,'L',false);
                $x = $pdf->GetX();
                $y = $pdf->GetY();      
        $pdf->SetXY($x+141, $y-5.5);
                $pdf->MultiCell(35,$alto,utf8_decode(''.$hasta),1,'L',false);
//$pdf->MultiCell(60,5,)
